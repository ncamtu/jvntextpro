...
public class Option {
	...
	try {
		...
		else if (strOpt.compareToIgnoreCase("model_file") == 0){
			modelFile = strVal;
		}
	    }
    ...
    public BufferedReader openModelFile() {
		...
		if (!modelFile.endsWith("zip")){			
			...
			try {
				...
				ZipEntry entry= zf.getEntry("model.txt");
	
				fin = new BufferedReader(new InputStreamReader(zf.getInputStream(entry), "UTF-8"));
				//zf.close();
				return fin;
			}
			catch (Exception ex) {
