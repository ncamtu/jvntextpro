...
public abstract class BasicContextGenerator extends ContextGenerator {
	...
	// common variables
	/** The cpnames. */
	protected Vector<String> cpnames;
	
	/** The paras. */
	protected Vector<Vector<Integer>> paras;
	
	/** The metadata of context predicates 
	 * second metadata: available for crfs to indicate this feature is 2nd order or 1st order
	 */
	protected Vector<Vector<Integer>> metas;
	...
	protected boolean readFeatureParameters(Element node){
		try{
			...
			metas = new Vector<Vector<Integer>>();
			...
			for (int i = 0; i < childrent.getLength(); i++)
				if (childrent.item(i) instanceof Element) {
					...
					Vector<Integer> meta = new Vector<Integer>();
					for (int j = 0; j < 2; ++j){
						meta.add(Integer.parseInt(parastr[j]));
					}
